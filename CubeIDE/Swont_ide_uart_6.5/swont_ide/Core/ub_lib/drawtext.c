...
int8_t FindGlyph(char c) {
  ...
  // return 127 if it found a space, there's no glyph available for space
  // character
  if (c == ' ') return 127;
...
/**
 * @fn Error DrawGlyph(int8_t, int16_t, int16_t, uint8_t, uint8_t, const
 * uint8_t*, const font_glyph_desc*)
...
Error DrawGlyph(int8_t index_glyph, int16_t x1, int16_t y1, uint8_t color,
                ...
                const font_glyph_desc *fontDesc) {
    ...
    for (int16_t xpos = 0; xpos < (width_px * fontSize); xpos++) {
      if (xpos & (7 + 8 * (fontSize - 1))) {
        if (fontSize == 1)
          ...
          byte_pixels <<=
              1 - (xpos % 2);  // bit shift left every other xpos increment to
                               // double size horizontally
      } else {
      ...
      if (byte_pixels & 0x80) UB_VGA_SetPixel(xpos + x1, ypos + y1, color);
    }

    // repeat line when font_size is 2 to double size vertically
    if (ypos % 2 == 0 && fontSize == 2) byte_index -= (7 + width_px) / 8;
  }
  return ERR_NONE;
}
...
Error DrawText(uint16_t x1, uint16_t y1, uint8_t color, char *textString,
  ...
  const font_glyph_desc *fontDesc =
      NULL;  // pointer to description of specified font bitmap array
